...
SELECT e.title, CONCAT (a.first_name, ' ', + a.last_name) AS nameActor
FROM episodes e 
...
SELECT 
	CONCAT (a.first_name," ", +a.last_name) AS nameActor
FROM actors a
JOIN actor_movie am
	ON a.id = am.actor_id
JOIN movies m
...
-- TT pratica individual

-- 1) Explique o conceito de normalização e para que é usado.
-- R: O conceito de normalização são algumas regras que existem para de deixar o projeto mais organizado, para reduzir as redundancias de dados e aumentar a integridade dos dados e melhorar o desempenho.
    
-- 2) adicione um filme á tabela de filmes.
INSERT INTO `movies` VALUES (23,NULL,NULL,'vingadores ultimato',9.8,17,'2019-04-25 00:00:00',182,null);

-- 3) Adicione um gênero à tabela de gêneros.
INSERT INTO `genres` VALUES (14,'2021-07-20 00:00:00',NULL,'Ação',14,1);

-- 4) Associe o filme do Ex 2. ao gênero criado no Ex. 3.
UPDATE movies SET genre_id = 14 WHERE movies.id = 23;

-- 5) Modifique a tabela de atores para que pelo menos um ator adicione como favorito o filme adicionado no Ex. 2.
UPDATE actors SET favorite_movie_id = 23 WHERE actors.id = 3;
...
-- R: Index é uma maneira mais eficiente quando se tem muitos dados no banco, onde associado a uma tabela ou exibição para acelerar a consulta de dados ou recuperação. 

-- 12) Crie um índice sobre o nome na tabela de filmes.
CREATE UNIQUE INDEX index_movies
ON movies (title);
